...
describe('<PowerSelect />', function () {
  ...
  it('should clear the selected option, when the backspace key is pressed on focussed powerselect', function () {
    ...
    powerselect.triggerKeydown(_constants.KEY_CODES.BACKSPACE);
    var args = powerselect.handleChange.getCall(0).args[0];
    expect(powerselect.handleChange.calledOnce).toBeTruthy();
    expect(args.option).toBe(undefined);
    expect(args.select).toBeTruthy();
  ...
  it('should not clear the selected option on backspace press if powerselect is disabled', function () {
    var handleOnFocus = _sinon2.default.spy();
    var wrapper = powerselect.renderWithProps({
      selected: _constants.countries[2],
      onFocus: handleOnFocus,
  ...
  it('should not clear the selected option on backspace press if powerselect does not have a clear option', function () {
    var handleOnFocus = _sinon2.default.spy();
    var wrapper = powerselect.renderWithProps({
      showClear: false,
      selected: _constants.countries[2],
